...
def create_album(request):
    # Initialize or retrieve selected files from the session
    selected_media_files_ids = request.session.get('selected_media_files', [])

    if request.method == 'POST':
        # Get the selected media file IDs from POST data, ignoring empty values
        selected_media_files_ids += [
            int(file_id) for file_id in request.POST.getlist('media_files') if file_id.isdigit() and int(file_id) not in selected_media_files_ids
        ]
        print("Selected media file IDs:", selected_media_files_ids)
        ...
        if album_form.is_valid() and media_form.is_valid():
            ...
            # Save media files linked to this album
            for file_id in selected_media_files_ids:
                media_file = UploadedFile.objects.get(id=file_id)
                AlbumMediaFiles.objects.create(album=album, file=media_file)

            # Clear selected files from the session after saving
            request.session.pop('selected_media_files', None)

            messages.success(request, 'Album created successfully!')
            return redirect('album_list')
        else:
            # Print errors for debugging if the forms are invalid
            if not album_form.is_valid():
                print("AlbumForm errors:", album_form.errors)
            if not media_form.is_valid():
                print("MediaFileSelectionForm errors:", media_form.errors)

            # Store the updated selected files in session if there was an error
            request.session['selected_media_files'] = selected_media_files_ids
    else:
        album_form = AlbumForm()
        media_form = MediaFileSelectionForm(user=request.user)

    # Pagination for media files
    media_files = UploadedFile.objects.filter(user=request.user)
    paginator = Paginator(media_files, 10)  # Show 10 media files per page
    page_number = request.GET.get('page')
    media_files_page = paginator.get_page(page_number)

    # Update session data for selected files
    request.session['selected_media_files'] = selected_media_files_ids

    return render(request, 'core/create_album.html', {
        'album_form': album_form,
        'media_form': media_form,
        'media_files_page': media_files_page,
        'previously_selected_files': set(selected_media_files_ids),  # Pass selected file IDs to the template
    })
